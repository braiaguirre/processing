...
int cant = 15000;  // amount of particles/lines
...
void setParticles() {
  ...
  for (int i = 0; i < particles.length; i++) { 
    float x = random(width);  // tira un x random
    float y = random(height);  // tira un y random
    ...
    particles[i]= new Particle(x, y, c);  // crea cant partículas/pixeles
  }
}
